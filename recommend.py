...
def find_closest(location, centroids):
    ...
    for i in range(1, len(centroids)):
        if distance(location, centroids[i]) < distance(location, closest_centroid):
...
def group_by_centroid(restaurants, centroids):
    ...
    for restaurant in restaurants:
        li.append(
                 [find_closest(restaurant_location(restaurant), centroids),
                  (restaurant)]
                 )
    return group_by_first(li)
...
def find_centroid(cluster):
    ...
    for restaurant in cluster:
        lat.append(restaurant_location(restaurant)[0])
        long.append(restaurant_location(restaurant)[1])
    mean_lat = mean(lat)
    mean_long = mean(long)
    centroids = [mean_lat, mean_long]
    return centroids
...
def k_means(restaurants, k, max_updates=100):
    ...
    while prev_centroids != centroids and n < max_updates:
        ...
        # BEGIN Question 6
        restaurants_grouped = group_by_centroid(restaurants, prev_centroids)
        centroids = []
        for cluster in restaurants_grouped:
...
def find_predictor(user, restaurants, feature_fn):
    ...
    x_values = [feature_fn(r) for r in restaurants]
    y_values = [reviews_by_user[restaurant_name(r)] for r in restaurants]
...
def best_predictor(user, restaurants, feature_fns):
    ...
    for func in feature_fns:
        predictors.append(list(find_predictor(user, reviewed, func)))
    
    def get_r(x):
        return x[1]
    
    best = max(predictors, key = get_r)
    return best[0]   
    # END Question 8
